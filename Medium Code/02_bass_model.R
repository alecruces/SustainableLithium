...
chl_ts <- ts(na.omit(final_data$chilean_exports), start = c(2014, 1), frequency = 4)

# Fit the Bass Model to Chilean exports
bm.ch.exp <- BM(chl_ts, display = FALSE)  # Fit the model
...
lines(
  ts(make.instantaneous(predict(GBM_chl_exp_mix, newx = c(1:(length(chl_ts) + 4)))), frequency = 4, start = 2014), col = "#00C3B1", lwd = 2, type = "l", lty = "dashed")
abline(v = 2023.5, col = "gray", lwd = 2, lty = 3)
...
bass_comp <- UCRCD(aus_ts, chl_ts, display = FALSE)
summary(bass_comp)
