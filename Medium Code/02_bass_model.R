...
full_quarters_df <- data.frame(
  ...
  quarter = as.character(full_quarters)    # Format as "YYYY Qn"
)
...
# Fit the Bass Model to Chilean exports
lit.chl.exp.quarter.kton <- na.omit(final_data$chilean_exports) # Remove NA values
ch.exp.ts <- ts(lit.chl.exp.quarter.kton, frequency = 4, start = 2014)

bm.ch.exp <- BM(lit.chl.exp.quarter.kton, display = TRUE)  # Fit the model
summary(bm.ch.exp)  # View model summary
...
plot(ch.exp.ts, type = "l", col = "black", lwd = 2,
     main = "Instantaneous Exports - Chile", xlab = "Quarter", ylab = "kT")
...
# Plot forecast
par(bg = "white")
plot(ch.exp.ts, ylab = "kT", main = "Forecast - Instantaneous Exports",
...
# Fit the Generalized Bass Model with mixed shocks
GBM_chl_exp_mix <- GBM(
  lit.chl.exp.quarter.kton, oos = 4, shock = "mixed", nshock = 2,
...
plot(ts.chl.comp, type = "l", col = "black", lwd = 2,
     main = "Instantaneous Exports with Shocks - Chile", xlab = "Quarter", ylab = "kT", xlim = c(2014, 2024))
lines(ts(make.instantaneous(GBM_chl_exp_mix$fitted), frequency = 4, start = 2014), col = "#00C3B1", lwd = 2, type = "l", lty = "dashed")
abline(v = 2023.5, col = "gray", lwd = 2, lty = 3)

## Step 5: Exploring Chile vs. Australia with the Competition Model

# Fit the Competition Model
lit.aus.exp.quarter.kton <- final_data$australian_exports[17:55] # Same time period as Chile

bass_comp <- UCRCD(lit.aus.exp.quarter.kton, lit.chl.exp.quarter.kton, display = TRUE)
summary(bass_comp)

ts.chl.comp <- ts(lit.chl.exp.quarter.kton, start=2014, frequency = 4)
ts.aus.comp <- ts(lit.aus.exp.quarter.kton, start=2014, frequency = 4)
...
lit.aus.exp.quarter.kton <- na.omit(final_data$australian_exports) # Remove NA values
aus.exp.ts <- ts(lit.aus.exp.quarter.kton, frequency = 4, start = 2010)

bm.aus.exp <- BM(lit.aus.exp.quarter.kton, display = TRUE)  # Fit the model
...
# Plot instantaneous data
plot(
  aus.exp.ts, type = "l", col = "black", lwd = 2,
  main = "Instantaneous Exports - Australia (Bass Model)", 
...
par(bg = "white")
plot(
  aus.exp.ts, ylab = "kT", main = "Forecast - Instantaneous Exports (Bass Model)", 
...
plot(
  lit.aus.exp$quarter, lit.aus.exp$lit_aus_exp_kton_met,  
  type = "l", col = "black", lwd = 2,
  main = "Instantaneous Exports - Australia (GBM)", 
  xlab = "Quarter", ylab = "kT", xlim = c(2010, 2024)
)

lines(
  ts(make.instantaneous(predict(GBM_aus_exp_rett2, newx = c(1:(length(lit.aus.exp.quarter.kton) + 4)))), 
